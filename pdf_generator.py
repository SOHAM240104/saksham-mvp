...
pdf_generator.py
================
Generate a PDF of RAG sources for Streamlit (inline base64 PDF).
Pipeline: Markdown → HTML → WeasyPrint.
Requires system libs (Pango, Cairo, GObject); on Streamlit Cloud use packages.txt.
"""
...
_HTML_CLASS = None  # lazy cache: weasyprint.HTML


def _get_weasyprint_html():
    """Load WeasyPrint once; raises if native libs are missing."""
    global _HTML_CLASS
    if _HTML_CLASS is not None:
        return _HTML_CLASS
    try:
        from weasyprint import HTML as HTML_

        _HTML_CLASS = HTML_
        return _HTML_CLASS
    except (OSError, ImportError) as e:
        raise RuntimeError(
            "WeasyPrint could not load (missing system libraries such as Pango/Cairo/GObject). "
            "On Streamlit Cloud, add the apt packages in packages.txt — see DEPLOY.md. "
            f"Original error: {e}"
        ) from e
...
def create_sources_pdf(docs: List[Document]) -> str:
    ...
    Returns the PDF as a Base64-encoded string for Streamlit inline display.
    """
